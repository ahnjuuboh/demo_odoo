...
class IssueTicket(models.Model):
  ...
  date = fields.Datetime(string='Ticket Date', default=lambda self: datetime.datetime.now, readonly=True)
  order_number = field.Integer(string='Order Number', required=True)
  order_id = fields.Integer(string='Order Id Number', compute='_compute_order_id', store=True)
  order_record = fields.Many2one(comodel='sale.order', string='Order', compute='_compute_order')
